...
import com.gsccs.oss.util.ServerHelper;
 ...
 */
public class OssServerAdaptorHandler extends SimpleChannelInboundHandler<DefaultFullHttpRequest> {
	private static final Logger log = LoggerFactory.getLogger(OssServerAdaptorHandler.class);
	...
	protected void channelRead0(final ChannelHandlerContext ctx, DefaultFullHttpRequest msg) throws Exception {
        ...
        final String filename = msg.headers().get("fileid"); 
        if(filename == null || "".equals(filename.trim())) { //没有文件名 直接返回4001 参数错误
        	...
        	try {
            	...
            	request1.setHeader("fileid", msg.headers().get("fileid"));
            	httpclient.start();

...
public class OssClient {
	...
	JSONObject putObject(String schema, String path, byte[] content)
		...
		if (resEntity != null) {
			try {
				...
				resultJsonObject = (JSONObject) JSON.toJSON(entityStringBuilder.toString());
				/*resultJsonObject = new JSONObject(
	 ...
	 * @throws Exception
	 */
	JSONObject putObject(ObjectMetadata metadata) throws Exception {
		JSONObject resultJsonObject = null;
		...
		if (resEntity != null) {
			try {
				...
				if (null != resultJsonObject
					...
					resultJsonObject.put("id", fileid);

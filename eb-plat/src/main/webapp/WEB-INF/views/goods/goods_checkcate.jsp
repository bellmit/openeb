...
<h3>发布平台商品</h3>
    ...
    <div class="wrapper_search">
        ...
        <div class="wp_confirm"> <span class="btn_confirm">
				<form method="post" action="${pageContext.request.contextPath}/goods/add">
			        <input name="cateid" id="class_id" value="" type="hidden">
    ...
    function tonextClass(id){
    	...
    	/* $('#t_id').val(valarray[2]); */
    	$('#dataLoading').show();
    	$.getJSON('${pageContext.request.contextPath}/category/child?id='+valarray[0],
    		function(data){
    			console.log(data);
        		if(data != null && data != "null" && data != ""){
    				...
    				for (i=0; i<jsonObj.length; i++) {
    					a+='<li onclick="selClass(this);" id="'+ jsonObj[i].id +'|'+class_div_id+'"><a href="javascript:void(0)"><span class="has_leaf">'+jsonObj[i].title+'</span></a></li>';
    				}
    ...
    $('#commoditya > a').unbind().click(function(){
    	$.getJSON('${pageContext.request.contextPath}/category/child?id='+$('#class_id').val(), function(data){
    		if (data != null && data != "null" && data != ""){
    			var jsonObj = data;
    			//var jsonObj = eval("(" + data.result + ")");
    			//+'|'+data.staple_id+'|'+data.type_id
                $('#commListArea > ul').prepend('<li nc_type="'+jsonObj.id+'"><span class="title">'+data.title+'</span><a class="del_unavailable" href="JavaScript:void(0);"></a></li>');
                $('#select_list_no').hide();

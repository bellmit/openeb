...
<body>
	<div class="easyui-layout" fit="true" border="false">
		<div data-options="region:'north',title:'平台商城查询'"
	...
	<script type="text/javascript">
		...
		function addFun() {
			var store_dialog_ = $('#dd').dialog({
				title : '注册店铺',
				width : 600,
				height : 400,
				closed : false,
				cache : false,
				href : basepath+'/store/dataform',
				modal : true,
				buttons : [ {
					text : '创     建',
					handler : function() {
						...
						{
							url : basepath+'/store/create',
							success : function(data) {
							var json = $.parseJSON(data);
							if (json.success) {
								$('#list_data').datagrid('load');
									store_dialog_.dialog('close');
								}
								$.messager.show({
									title : '提示',
									msg : result.msg,
		...
		function editFun() {
			$('#dd').dialog({
				...
				closed : false,
				cache : false,
				href : '${pageContext.request.contextPath}/store/dataform',
				modal : true,
				buttons : [ {
					text : '添加',
					handler : function() {
		...
		function addCateFun() {
			var checkedRows = $("#list_data").datagrid('getChecked');
			if (checkedRows.length == 1) {
				window.location.href = basepath+"/storecate?storeId="+checkedRows[0].id;
			}else if(checkedRows.length > 1) {
				$.messager.show({
					title : '提示',
					msg : '只能选择一个记录!',

...
<script type="text/javascript">
					...
					if (json.success) {
						...
						for(var i = 0; i < data.length; i ++) {
							...
							"<tr>"+
								...
								"<td><input type='text' vtype='required&amp;&amp;unsigned' value='"+storenum+"' key='store' name='storenum' size='10'></td>"+
								"<td><div class='fill'>"+
									"<input type='text' vtype='unsigned&amp;&amp;minpirce' id='typeId"+typeId+"' value='"+price+"' key='price' name='price' size='10'>"+
									//"<img src='${pageContext.request.contextPath}/resources/images/btn_edit.gif' align='absmiddle' app='desktop'><span class='edit lnk fs10' onclick='getMemberPrice("+typeId+")'>会员价</span>"+
									"</div></td>"+
								"<td><div class='fill'>"+
	...
	function createGoods() {
		...
		for(var i = 0; i < tbody.length; i++) {
			...
			$("#goods-table").append("<tbody class='productNode'><tr><td><input type='checkbox' class='pro-marketable-check' value='' state='' key='marketable' name='' checked='checked'></td>"+
							...
							"<td><input type='text' vtype='required&amp;&amp;unsigned' value='' key='store' name='storenum' size='10'></td>"+
	...
	$(function() {
		...
		$("#uploadify").uploadify({
			debug			: false, 
			swf 			: '${pageContext.request.contextPath}/resources/js/uploadify.swf',	//swf文件路径
			method			: 'post',	// 提交方式
			...
			buttonImage		: '${pageContext.request.contextPath}/resources/images/addGoods.png',	// 按钮的背景图片,会覆盖文字
		...
		$.ajax({
			...
			success : function(json) {
				if (json.success) {
					...
					for(var i = 0; i < data.length; i ++){
						if(data[i].id==bandid){
							$("#brand").append("<option class=\"brandOp\" selected=\"selected\" value=\""+data[i].id+"\">"+data[i].name+"</option>");
						}else{
	...
	function saveBaby() {
		...
		if(skus.length > 0) {
			for(var i = 0; i < skus.length; i++) {
				...
				var storenum = $(skus[i]).find("tr:eq(0) td:eq(3) input[name='storenum']").val();
				...
				var arrVal = ""
				if(skuid != null){
					 arrVal = specIds+"="+specStr+"="+barcode+"="+storenum+"="+price+"="+cost+"="+marketPrice+"="+goodsWeight+"="+goodsQuantity+"="+skuid;
				}else{
					 arrVal = specIds+"="+specStr+"="+barcode+"="+storenum+"="+price+"="+cost+"="+marketPrice+"="+goodsWeight+";"+goodsQuantity;
				}
		...
		if(propVals.length >0 ){
		  ...
		  $.ajax({
			...
			data : "product="+$("#product_editForm").serialize()+"&skuArray="+ skuArray +"&checkVals="+checkVals+"&inputVals="+inputVals+"&selectVals="+selectVals,
			type : 'post',
			dataType : 'json',
			success : function(json) {
				$.messager.show({
...
<style>
#product_editForm input,#product_editForm textarea{font-size:10px;}
</style>
			...
			<div class="FormWrap" style="background:none">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="liststyle data width1" style="border:none">
					<tbody>
						...
						<tr>
							<!-- <th>商品id：</th> td><input autocomplete="off" class="x-input " type="hideen"
								name="pId" id="pId" vtype="hidden" value="${product.pId}" >
								<input autocomplete="off" class="x-input " type="hideen"
								name="typeId" id="typeId" vtype="hidden" value="${cate.id}" ></td> -->
							<td><input  type="hidden"
								name="pId" id="pId" value="${product.pId}" >
								<input  type="hidden"
								name="typeId" id="typeId"  value="${cate.id}" ></td>
						</tr>
						<tr>
							<th>商品关键词：</th>
						...
						<tr>
							...
							<td><textarea type="textarea" class="x-input"
									...
									maxth="255" id="dom_el_8bbbb42">${product.brief}</textarea><br />
						...
						<tr>
							...
							<td>
								<div class="division pic-main">
									...
									<div class="pic-area"><!--  -->
										<input type="hidden" name="picUrl" id="picUrl" value="${imgurls}">
							...
							<tbody>
								...
								<tr>
									...
									<td><input autocomplete="off" class="x-input " type="text"
										...
										vtype="unsigned" value="${product.weight}"><span class="notice-inline"> 千克(kg)</span></td>
								</tr>
								<tr>
							...
							<td>
								<div id="ipt_dfc1a4" class="clearfix">
									<div id="dfc1a4" class="object-select clearfix"
										...
										<div class="object-select clearfix" id="gEditor-GCat-category">
											<div class="label" rel="请选择" onclick="chooseDeliverType();" style="float: left;padding:0;width:77px;height:23px;"><img alt="请选择" src="${pageContext.request.contextPath}/resources/images/choose.png"></div>
		...
		<div>
			...
			<div class="tableform">
				<button type="button" class="btn btn-has-icon">
					<span><span>
					<i class="btn-icon">
						<img src="${pageContext.request.contextPath}/resources/images/btn_add.gif"></i>添加配件</span></span>
				</button>
			...
			<div>
				...
				<div class="tableform">
					<div>
						<textarea name="desc" id="description" style="width:800px;height:400px;visibility:hidden;">
						<c:if test="${desc !=''}">
							${desc}
						</c:if>
						</textarea>

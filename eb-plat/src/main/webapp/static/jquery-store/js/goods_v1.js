...
var specGroupFun = function(){
	...
	$("[name=skuDo]").each(function(y,x){
		var oldSpec = {
			...
			goodsspecid:$(this).find("[name=goodsspecid]").val()
		};
		skuTrArr[y] = oldSpec;
	});
		...
		for(var s = 0; s < arr.length; s++){
			...
			for(var e = 0; e < skuTrArr.length; e++){
				if(skuTrArr[e].id == vaId){
					...
					var tr = '<tr name="skuDo" spId='+arr[s].spId+' spName='+arr[s].spName+' specValueName='+arr[s].specValueName+' specValueId='+arr[s].specValueId+' value='+arr[s].specValueId+'>'+
						...
						'<td><input class="text" name="huohao" type="text" value="'+skuTrArr[e].huohao+'"/><input type="hidden" name="goodsspecid" value="' + skuTrArr[e].goodsspecid + '" /></td>'+
						openTr +
						'</tr>';
				}
			}
			if(flg){
				var tr = '<tr name="skuDo" spId='+arr[s].spId+' spName='+arr[s].spName+' specValueName='+arr[s].specValueName+' specValueId="'+arr[s].specValueId+'" value="'+arr[s].specValueId+'">'+
					...
					'<td><input class="text" name="huohao" type="text"/><input type="hidden" name="goodsspecid" /></td>'+

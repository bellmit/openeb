...
public class OssClient {
	...
	public JSONObject putObject(ObjectMetadata metadata) throws Exception {
		JSONObject resultJsonObject = null;
		if (null == metadata || null == metadata.getSite() || metadata.getSite().trim().isEmpty() || null == metadata.getPath()
				|| metadata.getPath().trim().isEmpty() || null == metadata.getContent()) {
			resultJsonObject = new JSONObject();
			resultJsonObject.put("status", false);
			resultJsonObject.put("msg", "参数不完整");
			return resultJsonObject;
		...
		builder.addTextBody("schema", metadata.getSite());
		builder.addTextBody("path", metadata.getPath());
		builder.addTextBody("extension", metadata.getType().getType());
		...
		if (resEntity != null) {
			try {
				...
				if (null != resultJsonObject
					...
					resultJsonObject.put("url", metadata.getDomain() + metadata.getPath() + fileid
							+ metadata.getType().getType());
				}

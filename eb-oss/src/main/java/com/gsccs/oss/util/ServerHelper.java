...
public class ServerHelper {
	...
	static {
		try {
			...
			TMP_DIR = new File(ROOT_DIR.getAbsoluteFile() + File.separator
					+ "tmp");
			if (!TMP_DIR.exists())
				TMP_DIR.mkdir();
	...
	public static String getStorePath(String filename) {
		...
		sb.append(filename.substring(0, 2)).append(File.separator)
	...
	public static void deleteTmpFile(String filename) {
		try {
			File dir = new File(getDestDir().getAbsolutePath() + File.separator
